...
function withdraw(accountNumber: number, pin: number, amount: number): string {
...
function greetBasedOnTimeWithIfStatements(hours: number): string {
  let greeting: string;
...
function greetBasedOnTimeWithSwitchStatements(hours: number): string {
  let greeting: string;
  switch (true) {
...
console.log(
  `The greet Based On Time With If-statements is: ${greetBasedOnTimeWithIfStatements(
    currentHour
  )}`
);
console.log(
  `The greet Based On Time With Switch-Statements is: ${greetBasedOnTimeWithSwitchStatements(
    currentHour
  )}`
);
console.log("/**********************************/\n");
...
//Question#4
function getStudyLevelWithIfStatements(classNumber: number): string {
...
function getStudyLevelWithSwitchStatements(classNumber: number): string {
  let studyLevel: string;
...
console.log(
  `For class ${classNumber}, the study level is: ${getStudyLevelWithIfStatements(
    classNumber
  )}`
);
console.log(
  `For class ${classNumber}, the study level is: ${getStudyLevelWithSwitchStatements(
    classNumber
  )}`
);
console.log("/**********************************/\n");
...
//Question#5
function findGreatestAndSmallest(
  a: number,
  b: number,
  c: number,
  d: number,
...
const num1 = 15,
  ...
  num3 = 22,
  num4 = 10,
  num5 = 35; // Change these numbers as needed
const { greatest, smallest } = findGreatestAndSmallest(
  num1,
  num2,
  num3,
  num4,

...
const updateQuantity = (itemName: string, newQuantity: number): void => {
  const index = shoppingCart.findIndex((item) => item.name === itemName);
...
const removeNegativeNumbers = (numbers: number[]): number[] => {
  const nonNegativeNumbers = numbers.filter((number) => number >= 0);
  return nonNegativeNumbers;
};
const originalArray1: number[] = [1, -2, 3, -4, 5, -6];
...
const convertTemperatures = (celsiusTemperatures: number[]): number[] => {
...
const inputTemperatures: number[] = [20, 25, 30, 35];
...
const removeOddNumbers = (numbers: number[]): number[] => {
  const evenNumbers = numbers.filter((number) => number % 2 === 0);
  return evenNumbers;
};

const originalArray2: number[] = [1, 2, 3, 4, 5, 6, 7, 8, 9];
